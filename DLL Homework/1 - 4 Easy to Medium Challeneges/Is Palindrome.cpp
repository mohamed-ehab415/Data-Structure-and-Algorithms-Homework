...
//////////////   slove problem by solution       //////////////////////
bool is_panlindrome1(){ // with out lenght
        if (!head || head->next== nullptr)
            return true;
        Node*go =head ;
        Node *back=tail;
        for (; go->next!=tail || go == tail ;go=go->next,back=back->prev) {
            if (go->data!= back->data)
                return false;
...
void test1() {
    ...
    bool expected = false;
    bool result = list.is_panlindrome();
    if (expected != result) {
        cout << "no match:\nExpected: " <<
             expected << "\nResult  : " << result << "\n";
        assert(false);
